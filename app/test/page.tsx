...
export default function TypingTest() {
    ...
    if (typed.length >= currentText.length) {
      ...
      if (nextIndex < testTexts.length) {
        ...
        try {
          ...
          const additionalChars = newTexts.reduce(
  ...
  useEffect(() => {
    if (isTestActive) {
      ...
      timerRef.current = setInterval(() => {
        ...
        if (elapsedMinutes > 0) {
          const currentWpm = Math.round(
            totalWordsTypedRef.current / elapsedMinutes
          );
          setWpm(currentWpm);
  ...
  const moveToNextText = () => {
    ...
    const wordsInCurrentText = currentText
      .split(/\s+/)
  ...
  return (
    <div
      className={`container px-4 py-8 transition-colors duration-1000 md:py-12 ${
    ...
    >
      <div className="mb-8 flex items-center">
        ...
        <h1 className="solo-font glow-text text-3xl font-bold">Typing Test</h1>
      </div>

      {/* Test settings */}
      <Card className="mb-8 border-solo-purple/20 bg-solo-darkgray/80 p-6">
      ...
      {testCompleted ? (
        ...
        <Card
          className={`mb-8 border-solo-purple/20 bg-solo-darkgray/80 p-6 transition-all duration-500 ${getRankEffect()}`}
        >
          {!isTestActive ? (
            <div className="flex flex-col items-center justify-center py-12">
              ...
              <p className="mb-8 max-w-2xl text-center text-gray-300">
                ...
                {dungeonRanks.find((r) => r.id === selectedRank)?.label}-Rank
                difficulty.
              </p>
              <div className="flex flex-col items-center gap-4">
                <p className="animate-pulse text-lg font-semibold text-solo-purple-light">
                  Press Enter to start the test
                </p>
                <p className="text-sm text-gray-400">or</p>
                <Button
                  size="lg"
                  className="bg-solo-purple px-8 py-6 text-lg text-white hover:bg-solo-purple-dark"
                  onClick={startTest}
                >
                  Click to Start Test
                </Button>
            ...
            <div className="relative">
              ...
              <div className="mb-4">
                ...
                <Progress
                  ...
                  className="h-2"
                  indicatorClassName={
                    typingSpeed === 'fast' ? 'bg-blue-500' : 'bg-solo-purple'
                  }
                />
              </div>
              ...
              <div className="relative flex-1">
                ...
                <div className="mx-auto max-w-[800px] p-4">
                  <div
                    className="typing-text-container relative rounded-lg bg-solo-black/30 p-6"
                    style={{
                      minHeight: '150px',
                      maxHeight: '200px',
                      overflowY: 'auto',
      ...
      {isTestActive && (
        <Card
          className={`border-solo-purple/20 bg-solo-darkgray/80 p-6 transition-colors duration-500 ${
            typingSpeed === 'fast'
              ? 'border-blue-500/30'
              : 'border-solo-purple/20'
          }`}
        >
          <h2
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
            <div
              className={`rounded-md p-4 transition-colors duration-500 ${
                typingSpeed === 'fast' ? 'bg-blue-950/50' : 'bg-solo-black/50'
              }`}
            >
              <div className="mb-1 text-sm text-gray-300">Words Per Minute</div>
              <div
                className={`font-mono text-3xl ${typingSpeed === 'fast' ? 'text-blue-400' : 'text-solo-purple-light'}`}
              >
                {wpm}
              </div>
            </div>
            ...
            >
              <div className="mb-1 text-sm text-gray-300">Accuracy</div>
            ...
            >
              <div className="mb-1 text-sm text-gray-300">Characters Typed</div>
              <div className="font-mono text-3xl">{totalCharsRef.current}</div>
            </div>
          </div>
        </Card>
      )}
