...
describe('useTestHistory', () => {
  ...
  beforeEach(() => {
    ...
    (testResultsService.fetchHistory as any).mockResolvedValue(mockResults);
  });
  ...
  it('should set error when not authenticated', async () => {
    ...
    expect(result.current[0].error).toBe("Please log in to view your test history");
  });

  it('should set error when fetch fails', async () => {
    const testError = new Error('Failed to fetch');
    (testResultsService.fetchHistory as any).mockRejectedValue(testError);
  ...
  it('should allow manual refresh of history', async () => {
    ...
    (testResultsService.fetchHistory as any).mockResolvedValue(newResults);
