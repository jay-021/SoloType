...
export function useTypingTexts(
  options: UseTypingTextsOptions
): [TextProgressState, TextProgressActions] {
    ...
    if (!isActive) {
      try {
        ...
        if (texts.length > 0) {
          ...
          console.log(
            `Loaded ${texts.length} texts with ${total} total characters`
          );
        }
      } catch (error) {
  ...
  const moveToNextText = () => {
    ...
    console.log(
      `Moving to next text. Current index: ${currentTextIndex}, Total texts: ${testTexts.length}`
    );
    ...
    if (nextIndex < testTexts.length) {
      ...
      console.log(
        `Moving to text ${nextIndex}: "${nextText.substring(0, 30)}..."`
      );
    } else {
      ...
      try {
        ...
        if (nextIndex >= testTexts.length) {
          if (newTexts.length > 0) {
            ...
            console.log(
              `Set new current text: "${newTexts[0].text.substring(0, 30)}..."`
            );
          } else {
        ...
        console.log(
          `Updated total characters to ${totalCharacters + additionalChars}`
        );
      } catch (error) {
