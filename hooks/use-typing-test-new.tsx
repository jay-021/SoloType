...
export function useTypingTest(options: UseTypingTestOptions = {}): [TypingTestState, TypingTestControls] {
   ...
   * Handle typing input
   */
  const handleTyping = useCallback((e: React.ChangeEvent<HTMLTextAreaElement> | null, value: string = '') => {
    ...
    console.log('handleTyping called with value:', value.substring(0, 10) + (value.length > 10 ? '...' : ''));

    // Start timer on first keystroke if not started yet
    if (!isTimerStarted && value.length > 0) {
      console.log('Starting timer on first keystroke', { isTimerStarted, valueLength: value.length });
      timerControls.start();
    ...
    const correctChars = value.split('').filter((char, i) => 
      textState.currentText.charAt(i) === char
    ).length;
  ...
  }, [isTestActive, isTimerStarted, timerControls, timerState, typingStatsActions, textState]);
  ...
  const getRankEffect = () => {
    ...
    switch (rankInfo.id) {
      case "s": return "rank-effect-s";
      case "a": return "rank-effect-a";
      case "b": return "rank-effect-b";
      default: return "";
    }
  };
      ...
      <div className="text-lg leading-relaxed">
        {textState.currentText.split('').map((char, index) => {
          ...
          if (index < typedText.length) {
            ...
            className = "text-white border-b-2 border-solo-purple animate-pulse"; // Current position
          }
