...
export default function DynamicKeyboard({ activeKeys, typingSpeed }: DynamicKeyboardProps) {
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particlesRef.current.length; i++) {
        ...
        if (particlesRef.current[i].size <= 0.5 || particlesRef.current[i].alpha <= 0) {
  ...
  useEffect(() => {
    ...
    activeKeys.forEach((key) => {
      const keyElement = Array.from(keyElements).find(
        (el) => el.textContent?.toLowerCase() === key.toUpperCase() || (key === "space" && el.textContent === "Space"),
      ) as HTMLElement
      ...
      if (keyElement) {
        ...
        const particleColor = typingSpeed === "fast" ? "rgba(59, 130, 246, 0.8)" : "rgba(139, 92, 246, 0.8)"
  ...
  return (
    <div className="keyboard-container mt-8 relative">
      ...
      {rows.map((row, rowIndex) => (
        <div key={rowIndex} className="keyboard-row">
          {row.map((key) => {
            ...
            const glowClass = isActive ? (typingSpeed === "fast" ? "key-glow-blue" : "key-glow-purple") : ""

            return (
              <div key={key} className={`${keyClass} ${glowClass} transition-all duration-300`}>
