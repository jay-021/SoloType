...
export default function TestHistoryStats({ results }: TestHistoryStatsProps) {
  ...
  const stats = useMemo(() => {
    ...
    const timeSeriesData = sortedResults.slice(0, 15).reverse().map(result => ({
    ...
    const rankData = Object.entries(
      results.reduce((grouped, result) => {
        ...
        return grouped
      }, {} as Record<string, { tests: number; totalWpm: number; totalAccuracy: number }>)
    ).map(([rank, data]) => ({
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-2xl font-bold text-solo-purple">{Math.round(stats.bestWpm)}</CardTitle>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-2xl font-bold text-solo-purple">{Math.round(stats.bestAccuracy)}%</CardTitle>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-2xl font-bold text-solo-purple">{stats.bestWords}</CardTitle>
              ...
              <ResponsiveContainer width="100%" height="100%">
                ...
                >
                  ...
                  <XAxis 
                    ...
                    height={60}
                  />
                  <YAxis domain={[Math.max(70, Math.min(...stats.timeSeriesData.map(d => d.accuracy)) - 5), 100]} />
                  <Tooltip 
                    formatter={(value, name) => [
                      `${value} ${name === 'wpm' ? 'WPM' : '%'}`,
                      name === 'wpm' ? 'Words Per Minute' : 'Accuracy'
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Average WPM across difficulty levels</CardDescription>
          </CardHeader>
              ...
              >
                ...
                <Tooltip 
                  formatter={(value, name) => [
                    ...
                    name === 'avgWpm' ? 'Avg. Words Per Minute' : 'Avg. Accuracy'
                  ]}
